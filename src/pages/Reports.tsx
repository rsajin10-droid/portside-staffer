...
export default function Reports() {
  ...
  const getStatusLabel = (r: AttendanceRecord) => {
    ...
    if (r.status === 'extra_duty') return 'ED';
  ...
  const monthDriverStats = useMemo(() => {
    ...
    return {
      present: monthDriverData.filter(r => ['present', 'dcd', 'dcn'].includes(r.status)).length,
      absent: monthDriverData.filter(r => r.status === 'absent').length,
      extraDuty: monthDriverData.filter(r => r.status === 'extra_duty').length,
    };
  }, [monthDriverData]);
  ...
  const downloadMonthDriverPdf = () => {
    ...
    if (monthDriverStats) {
      doc.text(`Present: ${monthDriverStats.present} | Absent: ${monthDriverStats.absent} | Extra Duty: ${monthDriverStats.extraDuty}`, 14, 32);
    }
    autoTable(doc, {
      startY: 38,
      head: [['#', 'Date', 'Shift', 'Status']],
      body: monthDriverData.map(r => [r.serial, r.date, r.shift.toUpperCase(), r.statusLabel]),
    });
    doc.save(`driver_${staff?.name}_${MONTHS[month]}_${year}.pdf`);
  };

  const downloadAllDriversExcel = () => {
    const data = allDriversData.map(d => ({
      '#': d.serial,
      'Driver Name': d.name,
      'Mobile': d.mobile,
      'Present': d.present,
    ...
    <Layout>
      <div className="space-y-4 max-w-4xl mx-auto px-1">
            ...
            <CardContent className="space-y-3">
              ...
              {monthDriverStats && (
                <div className="flex gap-4 text-sm flex-wrap">
                  ...
                  <span className="text-warning font-medium">Extra Duty: {monthDriverStats.extraDuty}</span>
                </div>
              )}
              <div className="flex gap-2 flex-wrap">
                <Button size="sm" onClick={() => setShowReport(true)} disabled={!selectedDriver}><Eye className="h-4 w-4 mr-1" />View Report</Button>
                <Button size="sm" variant="outline" onClick={downloadMonthDriverPdf} disabled={!selectedDriver}><Download className="h-4 w-4 mr-1" />PDF</Button>
              </div>
            </CardContent>
          </Card>
        )}
        ...
        {showReport && mode === 'month_driver' && selectedDriver && (
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-base">{driverSearch} - {MONTHS[month]} {year} ({monthDriverData.length} records)</CardTitle>
            </CardHeader>
            <CardContent className="px-2 md:px-6">
              <div className="overflow-auto max-h-[500px]">
                <Table>
                  ...
                  <TableBody>
                    ...
                    {monthDriverData.length === 0 && (
                      <TableRow><TableCell colSpan={4} className="text-center text-muted-foreground text-xs">No records</TableCell></TableRow>
                    )}
                  </TableBody>
                </Table>
              </div>
        ...
        {showReport && mode === 'all_drivers_custom' && (
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-base">All Drivers: {dateFrom} to {dateTo} ({allDriversData.length} drivers)</CardTitle>
            </CardHeader>
            <CardContent className="px-2 md:px-6">
              <div className="overflow-auto max-h-[500px]">
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead className="text-xs">ED</TableHead>
                      <TableHead className="text-xs">Total</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {allDriversData.map(d => (
                      <TableRow key={d.id}>
                        <TableCell className="text-xs py-1.5">{d.serial}</TableCell>
                        <TableCell className="text-xs font-medium py-1.5">{d.name}</TableCell>
                        <TableCell className="text-xs py-1.5 hidden sm:table-cell">{d.mobile}</TableCell>
                        <TableCell className="text-xs py-1.5 text-success font-medium">{d.present}</TableCell>
                        <TableCell className="text-xs py-1.5 text-destructive font-medium">{d.absent}</TableCell>
                        <TableCell className="text-xs py-1.5 text-warning font-medium">{d.ed}</TableCell>
                        <TableCell className="text-xs py-1.5 font-bold">{d.present + d.absent + d.ed}</TableCell>
                      </TableRow>
                    ))}
                    {allDriversData.length === 0 && (
                      <TableRow><TableCell colSpan={7} className="text-center text-muted-foreground text-xs">No records</TableCell></TableRow>
                    )}
                  </TableBody>
                </Table>
              </div>
